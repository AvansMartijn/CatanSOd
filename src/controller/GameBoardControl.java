			...
			while (strLocArr.size() > strLocCount) {
				...
				for (StreetLocation sl : streetLocArr) {
					...
					if (strLocArr.get(strLocCount).getBlStart() == sl.getBlStart()
						...
						sl.getBlStart().getAdjecentStreets().add(sl);
						sl.getBlEnd().getAdjecentStreets().add(sl);
						break;
					}
				}
				...
				if (!exists) {
					...
					strLocArr.get(strLocCount).getBlStart().getAdjecentStreets().add(strLocArr.get(strLocCount));
					strLocArr.get(strLocCount).getBlEnd().getAdjecentStreets().add(strLocArr.get(strLocCount));
				}
